...
class RoombaBehaviorTree(BehaviorTree):
    ...
    def __init__(self):
        super().__init__()
        self.root = SelectorNode("Raiz")
        s1 = SequenceNode("Sequence1")
        s2 = SequenceNode("Sequence2")
        foward = MoveForwardNode()
        spiral = MoveInSpiralNode()
        backward = GoBackNode()
        rotate = RotateNode()
        s1.add_child(foward)
        s1.add_child(spiral)
        s2.add_child(backward)
        s2.add_child(rotate)
        self.root.add_child(s1)
        self.root.add_child(s2)
